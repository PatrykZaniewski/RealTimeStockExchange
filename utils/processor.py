...
def read_specified_cloud_timestamp(filename, processing_timestamps: Dict[str, ProcessingTimestamps]):
    ...
    for obj in data:
        ...
        setattr(related_processing_timestamp, field_name, timestamp)
...
def generate_processing_results(timestamps: List[ProcessingTimestamps]):
    ...
    for timestamp in timestamps:
        results.append(ProcessingResults(
            ...
            stock_core_processing=int(((timestamp.stock_core_status_sending + timestamp.stock_core_status_send)/2) - timestamp.stock_core_order_received),
            stock_core_broker_order_status_collector_communication=int(timestamp.broker_order_status_collector_status_received - ((timestamp.stock_core_status_sending + timestamp.stock_core_status_send)/2)),
